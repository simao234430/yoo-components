...
fs.outputFile('../../../../icon/index.es.js', entryCode, (err) => {
  if (err) return;
  console.log('Generate es module entry success!'); // eslint-disable-line
});
fs.outputFile('../../../../icon/index.js', babel.transform(entryCodeCjs, babelConfigCjs).code, (err) => {
  if (err) return;
...
fs.outputFile('../../../../icon/react-icon/context.js', contextJsx, (err) => {
  if (err) return;
  console.log('Generate context file success!'); // eslint-disable-line
});

fs.outputFile(
  '../../../../icon/react-icon-cjs/context.js',
...
fs.outputFile('../../../../icon/react-icon/context.d.ts', dTs, (err) => {
  if (err) return;
  console.log('Generate context ts file success!'); // eslint-disable-line
});

fs.outputFile('../../../../icon/react-icon-cjs/context.d.ts', dTs, (err) => {
  if (err) return;
  ...
  for (let i = 0; i < svgDataFlat.length; i++) {
    ...
    nunjucks.render(
      ...
      (err, res) => {
        ...
        fs.outputFile(
          `../../../../icon/${cjs ? 'react-icon-cjs' : 'react-icon'}/${iconName}/index.js`,
          code,
          (err) => {
            if (!err) {
              length += 1;
...
fs.outputFileSync('../../../../icon/index.d.ts', typingsCode);
